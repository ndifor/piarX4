CREATE TABLE IF NOT EXISTS `PiarX4_stock_locations` (
...
UPDATE `PiarX4_item_quantities` SET location_id = (SELECT MIN(`location_id`) FROM `PiarX4_stock_locations`);

ALTER TABLE `PiarX4_inventory`
 ADD COLUMN trans_location int(11);

UPDATE `PiarX4_inventory` SET trans_location = (SELECT MIN(`location_id`) FROM `PiarX4_stock_locations`);
...
-- ALTER TABLE PiarX4_items DROP COLUMN location;

ALTER TABLE `PiarX4_receivings_items`
 ADD COLUMN item_location int(11);
 
UPDATE `PiarX4_receivings_items` SET item_location = (SELECT MIN(`location_id`) FROM `PiarX4_stock_locations`);

ALTER TABLE PiarX4_receivings_items
 MODIFY COLUMN `item_location` INT(11) NOT NULL,
 ADD KEY `item_location` (`item_location`),
 ADD CONSTRAINT `PiarX4_receivings_items_ibfk_3` FOREIGN KEY (`item_location`) REFERENCES `PiarX4_stock_locations` (`location_id`);
 
ALTER TABLE `PiarX4_sales_items`
 ADD COLUMN `item_location` int(11);

UPDATE `PiarX4_sales_items` SET item_location = (SELECT MIN(`location_id`) FROM `PiarX4_stock_locations`);
...
UPDATE `PiarX4_sales_suspended_items` SET item_location = (SELECT MIN(`location_id`) FROM `PiarX4_stock_locations`);
